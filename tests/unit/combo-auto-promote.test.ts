...
test("promoteSuccessfulComboModel persists the reordered combo when flag is on", async () => {
  ...
  const ok = await promoteSuccessfulComboModel(makeCombo(), "b/2", { comboAutoPromoteEnabled: true }, {
...
test("promoteSuccessfulComboModel is a no-op when the flag is off", async () => {
  let called = false;
  const ok = await promoteSuccessfulComboModel(makeCombo(), "b/2", { comboAutoPromoteEnabled: false }, {
    updateCombo: async (id, data) => {
      called = true;
      return data;
    },
...
test("promoteSuccessfulComboModel is a no-op when the winner is already first", async () => {
  let called = false;
  const ok = await promoteSuccessfulComboModel(makeCombo(), "a/1", { comboAutoPromoteEnabled: true }, {
    updateCombo: async (id, data) => {
      called = true;
      return data;
    },
